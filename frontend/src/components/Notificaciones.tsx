...
export default function Notificaciones() {
  ...
  // Al abrir marcar todas como vistas
  const handleAbrir = () => {
        ...
        <div className="absolute right-0 top-10 w-80 bg-white rounded-xl border border-gray-200 shadow-lg z-50">
          <div className="p-4 border-b border-gray-100 flex items-center justify-between">
            <h3 className="font-semibold text-gray-800 text-sm">
              Licitaciones nuevas
              <span className="ml-2 text-xs text-gray-400">ultimos 7 dias</span>
            </h3>
            {total > 0 && (
              <button
                onClick={limpiar}
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 border-b border-gray-50 transition-colors ${
                      esVista ? "hover:bg-gray-50" : "bg-blue-50 hover:bg-blue-100"
                    }`}
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 line-clamp-1">{item.titulo}</p>
                        <p className="text-xs text-gray-500 mt-0.5">{item.organismo}</p>
          ...
          {total > 0 && (
            <div className="p-3 border-t border-gray-100">
              <button
                onClick={() => { navigate("/licitaciones"); setAbierto(false); }}
                className="w-full text-center text-xs text-primary-600 font-medium"
              >
